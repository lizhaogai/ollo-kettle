...
Helper.prototype.delay = function (interval) {
    var timeout = 0;
...
Helper.prototype.data = function (node, data, callback) {
    if (!Array.isArray(data)) data = [data];
    async.each(data, function (item, callback) {
        var dData = (_.assign({ G: "0", V: 0, D: 50004, timestamp: Date.now() }, item));
        napp.rekuest('node.data').prop('node', node).params({nid: node.nid}).payload(dData).send(callback);
    }, callback);
};
...
//        }
    ...
    napp.phase(sira.boot.module('ollo-pusher'));
    napp.setAll(options);
    return napp;
};
...
//        test.nino.enable('resultful');
        test.napp.boot(function (err) {
            ...
            kettle.init(test.napp);
            done();
        });
    }
};

exports.setupNode = function setupNode(nid, cb) {
    return function (done) {
        var test = this;
        this.napp.rekuest('node.activate')
            .params({nid: nid})
            .prop('accessToken', test.token)
            .send(function (err, result) {
                t.notOk(err);
                t.property(result, 'token');
                test.Node.one({where: {nid: nid, ownerId: test.user.id}}, function (err, node) {
                    t.notOk(err);
                    t.ok(node);
                    t.propertyVal(node, 'token', result.token);
                    test.node = node;
                    cb(node);
                    done();
                });
